...
{
  {
    '159.203.75.200', // ipv4, ipv6, or hostname
    port: 1080,
    type: 5 // Proxy version (4 or 5). For v4a, just use 4.

    // Optional fields
    'some username', // Used for SOCKS4 userId auth, and SOCKS5 user/pass auth in conjunction with password.
    password: 'some password' // Used in conjunction with userId for user/pass auth for SOCKS5 proxies.
  },

  'connect', // connect, bind, associate

  destination: {
    '192.30.253.113', // ipv4, ipv6, hostname. Hostnames work with v4a and v5.
    port: 80
  },

  // Optional fields
  30000, // How long to wait to establish a proxy connection. (defaults to 30 seconds)
...
```typescript
{
  0, // The frame number (used for breaking up larger packets)
  ...
  <Buffer 01 02 03 04...> // A Buffer instance of data to include in the packet (actual data sent to the remote host)
}
...
```typescript
{
  net.Socket, // The underlying raw Socket
  remoteHost: {
    '1.2.3.4', // The remote host that is listening (usually the proxy itself)
